              ...
              myPosts.map((post) => (
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <h3 className="text-2xl font-bold text-gray-800 w-2/3 truncate">
                      {post.title}
                    </h3>
  ...
  const fetchProfile = async () => {
    try {
      ...
      const followRes = await fetch(
        `http://localhost:8080/api/follow/${username}/status`,
        {
          credentials: "include",
        }
      );
                  ...
                  onClick={async () => {
                    try {
                      const res = await fetch(
                        `http://localhost:8080/api/follow/${profile.username}`,
                        {
                          method: "POST",
                          credentials: "include",
                        }
            ...
            <div className="mt-4">
              ...
              <div className="flex flex-wrap gap-2">
                {skillsArray.map((skill) => (
                  <span
                    key={skill}
                    className="bg-violet-200 text-violet-800 px-3 py-1 text-xs rounded-full"
                  >
                    {skill}
                  </span>
                ))}
